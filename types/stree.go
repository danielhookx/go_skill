...
type SearchTree[T Orderliness] struct {
	root *TreeNode[T]
}
...
func (r *SearchTree[T]) del(pre, n *TreeNode[T]) {
	// node have no child
	if IsNil[*TreeNode[T]](n.left) && IsNil[*TreeNode[T]](n.right) {
		if pre.left == n {
			pre.left = nil
		}
		if pre.right == n {
	...
	if IsNil[*TreeNode[T]](n.right) && !IsNil[*TreeNode[T]](n.left) {
		next = n.left
	}
	if !IsNil[*TreeNode[T]](next) {
		if pre.left == n {
			pre.left = next
		}
		if pre.right == n {
